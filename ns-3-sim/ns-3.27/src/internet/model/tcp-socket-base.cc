...
//                   MakeEnumChecker (CLOSED, "Closed"))
    ...
    .AddTraceSource ("Latency",                                           // addition patch!
                     "Latency of current rx packet",
                     MakeTraceSourceAccessor (&TcpSocketBase::m_latency),
                     "ns3::Time::TracedValueCallback")
    .AddTraceSource ("RxAck",                                             // addition patch!
                     "Current Rx Ack No (only for latency recording)",
                     MakeTraceSourceAccessor (&TcpSocketBase::m_rxAck),
                     "ns3::SequenceNumber32TracedValueCallback")
    ...
    .AddTraceSource ("Rx",
                     ...
                     "ns3::TcpSocketBase::TcpTxRxTracedCallback")
    .AddTraceSource ("TcpRx",                                             // additional patch
                     "Receive tcp packet, for per-packet latency",
                     MakeTraceSourceAccessor (&TcpSocketBase::m_tcprxTrace),
...
{
  m_tcprxTrace (packet);      // actually not sure about the index tag
  ...
  if (!m.IsZero ())
    {
      ...
      // addition patch: record per packet latency
      m_rxAck = tcpHeader.GetAckNumber();
      m_latency = m;
      // NS_LOG_UNCOND ("Rx Ack No. " << m_rxAck.Get().GetValue() << ", latency: " << m.GetSeconds());
    }
}
