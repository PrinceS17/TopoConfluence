...
void
BriteTopologyHelper::SetQueue (std::string type)
{
  NS_LOG_FUNCTION (this);

  for (uint32_t i = 0; i < m_netDevices.size (); i ++)
  {
    BriteTopologyHelper::BriteEdgeInfoList::iterator it = m_briteEdgeInfoList.begin () + i;
    TrafficControlHelper red_tch;
    red_tch.SetRootQueueDisc("ns3::RedQueueDisc", 
                             "MinTh", DoubleValue(5),
                             "MaxTh", DoubleValue(15),
                             "QueueLimit", UintegerValue(100),
                             "LinkBandwidth", DataRateValue (DataRate ((*it).bandwidth * mbpsToBps)),
                             "LinkDelay", TimeValue (Seconds ((*it).delay / 1000.0)));
    red_tch.Install (*m_netDevices[i]);
  }
}
