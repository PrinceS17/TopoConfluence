...
void FNSSSimulation::assignIPv4Addresses(const Ipv4Address &base) {
	...
	Ipv4InterfaceContainer ifc;		// for debug only
	while(it != this->m_links.end()) {
		ifc = address.Assign(*it);
		for (auto j : {0, 1})
			m_map[ifc.GetAddress (j)] = it->Get (j)->GetNode ();

		// std::cout << it->Get (0)->GetNode ()->GetId () << ": " << ifc.GetAddress (0) << " --> " 
		// 	<< it->Get (1)->GetNode ()->GetId () << ": " << ifc.GetAddress (1) << std::endl;
		address.NewNetwork();
		it++;
	}
}

std::map <Ipv4Address, Ptr<Node>> FNSSSimulation::getIpMap () const {
	return m_map;
}

Ptr<Node> FNSSSimulation::getNodeByIp (Ipv4Address addr) const {
...
void FNSSSimulation::buildTopology(const fnss::Topology &topology) {
	...
	Ipv4GlobalRoutingHelper rh;
	internet.SetRoutingHelper (rh);			// my addition for routing downcast
	...
	for(std::set<std::pair <std::string, std::string> >::iterator it = edges.begin();
		...
		red_tch.SetRootQueueDisc("ns3::RedQueueDisc", 
                             	 "MinTh", DoubleValue(5),
                             	 "MaxTh", DoubleValue(15),
                             	 "QueueLimit", UintegerValue(60),
                             	 "LinkBandwidth", StringValue(edge.getCapacity().toString()),
                             	 "LinkDelay", StringValue(edge.getDelay().toString()));
		red_tch.Install (p2pDev);
